...
typedef actionlib::SimpleActionServer<learning_custom_action::AddIntsAction>
    Server;

void cb(const learning_custom_action::AddIntsGoalConstPtr &goal,
        Server *server) {
  // 获取目标值
  int num = goal->num;
  ROS_INFO("目标值:%d", num);
  // 累加并响应连续反馈
  int result = 0;
  learning_custom_action::AddIntsFeedback feedback;  // 连续反馈
...
int main(int argc, char *argv[]) {
  ...
  /*SimpleActionServer(ros::NodeHandle n,
     ...
     demo01_action::AddIntsGoalConstPtr &)> execute_callback, bool auto_start)
  */
  // actionlib::SimpleActionServer<demo01_action::AddIntsAction> server(....);
  // 参数含义：name, execute_callback, auto_start
