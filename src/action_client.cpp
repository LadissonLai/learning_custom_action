...
*/
typedef actionlib::SimpleActionClient<learning_custom_action::AddIntsAction>
    Client;

// 处理最终结果
void done_cb(const actionlib::SimpleClientGoalState &state,
             const learning_custom_action::AddIntsResultConstPtr &result) {
...
// 处理连续反馈
void feedback_cb(
    const learning_custom_action::AddIntsFeedbackConstPtr &feedback) {
...
int main(int argc, char *argv[]) {
  ...
  ros::NodeHandle nh;
  // 4.创建action客户端对象;
  // SimpleActionClient(ros::NodeHandle & n, const std::string & name, bool
  // spin_thread = true)
  ...
  /*
     ...
     const demo01_action::AddIntsResultConstPtr &result)> done_cb,
          ...
          boost::function<void (const demo01_action::AddIntsFeedbackConstPtr
     &feedback)> feedback_cb)
